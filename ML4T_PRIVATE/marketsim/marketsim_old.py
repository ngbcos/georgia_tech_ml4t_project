...
def compute_portvals(orders_file = "./orders/orders.csv", start_val = 1000000, commission=9.95, impact=0.005):
    ...
    #let's go through and do all the orders and get it over with
    for row in orders_df.itertuples(index=True):
        ...
        #calculate the cash hit you just took due to commissions and stuff
            #get the stock price at execution

        #uncomment for future debugging
    ...
    s = pd.Series(current_values_dictionary, name='PortVal').to_frame()
    s.index.name = 'Date'
    s['Date'] = s.index #make a column for fun

    print s

    uber_set = port_val_ledger.combine_first(s)
    ...
    #portvals = get_data(['IBM'], pd.date_range(order_date, order_date))
    #portvals = portvals[['IBM']]  # remove SPY

    #append an entry to our dictionary of dates and values
