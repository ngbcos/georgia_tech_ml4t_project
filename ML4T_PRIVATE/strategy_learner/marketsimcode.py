...
def compute_portval_stats(portvals, rfr=0.0, sf=252, sv=100000):
    ...
    # print("numerator ", (daily_returns - daily_rfr).std())
    # print("denominator ", (daily_returns - daily_rfr).mean())
    sr = (daily_returns - daily_rfr).mean() / (daily_returns - daily_rfr).std()
...
def compute_portvals_abridged_faster(orders_df, start_val=100000, commission=0.0, impact=0.0, stock_price_library = None):
    #this is a faster version of compute_portvals which assumes the order dates are identical to trading dates

    #orders_df = orders_df.sort_values(['Date']) #this sorts our dates
    orders_column = orders_df['Date']

    # Add a portval on the right
    port_val_ledger = pd.DataFrame(index=stock_price_library.index.copy()) #this comes back sorted
    # print port_val_ledger
    port_val_ledger['Date'] = port_val_ledger.index
    port_val_ledger['portvals'] = np.nan
    ...
    for date in list_of_dates:
        #if (date == orders_df['Date']).any(): #this is an expensive check.
            # print ('An order exists on this date so I should run through my orders')
        if orders_df.loc[date]["Shares"] != 0: #if there is an order for shares on this date, then execute and calculate
            current_stocks_dictionary, \
            current_values_dictionary, \
            cash = process_orders_on_date(cash, date, orders_df, stock_price_library,
                                          current_stocks_dictionary, current_values_dictionary, commission, impact)
